...
function generateFortunePdf({ order, saju, content }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // ---------- 월별운세 (있을 때만) ----------
      let sectionCounter = (content.sections || []).length;
      if (content.monthly && content.monthly.length) {
        sectionCounter += 1;
        ensureSpace(doc, 140);
        drawSectionHeading(doc, `${sectionCounter}. ${content.monthlyLabel || '월별 운세 흐름'}`);
        content.monthly.forEach((m) => drawMonthlyBlock(doc, m));
      }

      // ---------- 추천 날짜 리스트 (택일 상품 전용, 있을 때만) ----------
      if (content.recommendedDates && content.recommendedDates.length) {
        sectionCounter += 1;
        ensureSpace(doc, 140);
        drawSectionHeading(doc, `${sectionCounter}. 추천 날짜`);
        content.recommendedDates.forEach((d) => drawDateBlock(doc, d, false));
      }
      if (content.avoidDates && content.avoidDates.length) {
        sectionCounter += 1;
        ensureSpace(doc, 100);
        drawSectionHeading(doc, `${sectionCounter}. 피해야 할 날짜`);
        content.avoidDates.forEach((d) => drawDateBlock(doc, d, true));
      }
...
function drawTocPage(doc, content) {
  doc.font('extrabold').fontSize(26).fillColor(COLOR.navy);
  ...
  if (content.monthly && content.monthly.length) { n += 1; items.push(`${n}. ${content.monthlyLabel || '월별 운세 흐름'}`); }
  if (content.recommendedDates && content.recommendedDates.length) { n += 1; items.push(`${n}. 추천 날짜`); }
  if (content.avoidDates && content.avoidDates.length) { n += 1; items.push(`${n}. 피해야 할 날짜`); }
  if (content.closing) items.push(`${items.length + 1}. 마무리 총평`);

  items.forEach((label) => {
    const y = doc.y;
    doc.font('semibold').fontSize(16.5).fillColor(COLOR.ink);
    doc.text(label, MARGIN + 6, y, { width: CONTENT_W - 12 });
    doc.moveTo(MARGIN, doc.y + 9).lineTo(PAGE_W - MARGIN, doc.y + 9).lineWidth(0.5).stroke('#D8CDA8');
    doc.moveDown(1.15);
  });
}
...
function stripHanja(text) {
  return String(text || '')
    .replace(/[\u4E00-\u9FFF\u3400-\u4DBF]/g, '')
    .replace(/[()（）]\s*[()（）]/g, ''); // 한자만 있던 빈 괄호(예: "금()") 잔여물 제거
}
...
// 택일 상품 전용: 날짜(요일 자동 계산) + 짧은 이유 한 줄. avoid=true면 피해야 할 날짜 톤(빨강)으로 표시
function drawDateBlock(doc, item, isAvoid) {
  ensureSpace(doc, 56);
  const y = doc.y + 4;
  ...
  if (!isNaN(parsed)) {
    ...
    const w = WEEKDAY_KR[parsed.getDay()];
    dateLabel = `${m}/${d}(${w})`;
  }

  doc.roundedRect(MARGIN, y, 64, 22, 3).fill(isAvoid ? COLOR.red : COLOR.navy);
  doc.font('semibold').fontSize(10.5).fillColor(isAvoid ? '#F0D9D9' : COLOR.gold);
  doc.text(dateLabel, MARGIN, y + 6, { width: 64, align: 'center' });

  doc.font('regular').fontSize(11.5).fillColor(COLOR.ink);
  doc.text(stripHanja(item.reason || ''), MARGIN + 76, y + 3, { width: CONTENT_W - 76, lineGap: 5 });
  doc.moveDown(0.55);
}
