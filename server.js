...
- 지정된 분량을 정확히 채우되 내용을 억지로 반복하거나 늘리지 말 것`;
    ...
    const FORTUNE_FOCUS_NOTES = {
      ...
      lifetime: '- "평생사주" 상품이므로 재물운·결혼운(애정운)·건강운·직업운·인간관계를 각각 독립된 섹션으로 명확히 다룰 것 — 뭉뚱그리지 말고 항목별로 구체적으로 풀어낼 것',
    };
...
${isDeep ? `- ${monthlyKeyName} 각 항목: ${isLifetime ? '90~98자씩' : '70~85자씩'}, 12개 전부 채울 것
- closing: ${isLifetime ? '240~260자' : '200~260자'}` : `- closing: 120~180자`}
- 빈 문장이나 늘어지는 수사로 채우지 말고, 지정된 자리수 안에서 내용을 실제로 촘촘하게 채울 것 (여백 없이 알찬 문장으로)

작성 지침:
- 존댓말, 따뜻하지만 구체적인 어조
- 한자(漢字)는 절대 쓰지 말 것 — 오행(목·화·토·금·수), 육친 등 명리학 용어도 반드시 한글로만 표기 (PDF 폰트가 한자를 지원하지 않아 깨져 보임)
${focusNote ? focusNote + '\n' : ''}${isDeep ? (isLifetime ? `- sections는 아래 7개 구성으로 작성 (각각 소제목을 명확히 달 것):
  1) 타고난 사주 원국과 성격 풀이
  2) 재물운 — 평생에 걸친 재물의 흐름과 시기별 특징
  3) 결혼운/애정운 — 인연이 들어오는 흐름과 결혼 생활
  4) 건강운 — 특히 주의해야 할 시기와 관리 포인트
  5) 직업운/사회적 성취 — 어떤 분야·방식이 잘 맞는지
  6) 인간관계와 대인운
  7) 전체적으로 활용하면 좋은 점과 주의할 점
- ${monthlyKeyName}는 반드시 12개 모두, 위에 제시된 12개 인생 시기 그대로 채울 것` : `- sections는 아래 4개 구성으로 작성:
  1) 타고난 사주 원국 풀이 (일간 중심 성향 분석)
  2) ${productName}와 직접 관련된 심층 해석
  3) 올해~내년 흐름 (대운/세운 관점, 시기별 조언)
  4) 주의할 점과 활용하면 좋은 점
- ${monthlyKeyName}는 반드시 12개 모두, 1월부터 12월까지 채울 것`) : `- sections는 3개로 구성 (예: ①타고난 성향과 사주 원국 ②${productName} 심층 해석 ③주의할 점과 활용하면 좋은 점), 소제목도 함께 작성`}
